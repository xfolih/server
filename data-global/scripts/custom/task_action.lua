	...
	if completion and data.rewards then  
		if player:getStorageValue(taskOptions.bonusReward) >= 1 then  
			...
			for _, info in pairs (data.rewards) do  
				if info.randomSoulCore then  
					local randomIndex = math.random(1, #info.randomSoulCore)  
					local selectedSoulCore = info.randomSoulCore[randomIndex]  
					player:addItem(selectedSoulCore, 1)  
					window:addChoice("- 1 " .. ItemType(selectedSoulCore):getName())  
					...
					if taskOptions.selectLanguage == 1 then  
						player:say('Soul Core: ' .. ItemType(selectedSoulCore):getName(), TALKTYPE_MONSTER_SAY)  
					else  
						player:say('Random Soul Core: ' .. ItemType(selectedSoulCore):getName(), TALKTYPE_MONSTER_SAY)  
					end  
				elseif info[1] == "exp" then  
...
function confirmTaskModalWindow(player, storage)  
	...
	if data.rewards then  
		...
		if player:getStorageValue(taskOptions.bonusReward) >= 1 then  
			for _, info in pairs(data.rewards) do  
				if info.randomSoulCore then  
					window:addChoice("- 1 Random Soul Core")  
				elseif info[1] == "exp" then  
					if taskOptions.selectLanguage == 1 then  
						window:addChoice(task_pt_br.choiceText ..""..info[2]*taskOptions.bonusRate)  
					else  
